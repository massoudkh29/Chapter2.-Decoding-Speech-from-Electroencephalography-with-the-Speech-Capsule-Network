...
def network_model(X_Train, X_Test, y_train, y_test, n_chans, input_time_length, cuda, args, p, q):
    max_epochs = 30
    ...
    # model_test.run()
    model_test = test(model=eval_model, data=(x_test, y_test), args=args, p=p, q=q)
    return model_test
        ...
        for i, p in enumerate(word_id):
            ...
            for j, q in enumerate(word_id):
                if j != i:
                    ...
                    run_model = network_model(X_Train, X_Test, y_train, y_test, n_chans, input_time_length, cuda, args, p, q)
