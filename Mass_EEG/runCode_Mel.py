...
import numpy as np
from Mass_EEG.utils import load_subject_eeg, eeg_to_3d, return_indices, Ciaran, Experiment, balanced_subsample,mfcc_f
...
def network_model(X_Train, X_Test, y_train, y_test, n_chans, input_time_length, cuda, args):
    max_epochs = 30
    ...
    x_train = X_Train
    x_test = X_Test
    # x_train = x_train.reshape(-1, 25, 25, 5, 1)
    # x_test = x_test.reshape(-1, 25, 25, 5, 1)
    ...
    model_test = test(model=eval_model, data=(x_test, y_test), args=args)
...
if __name__ == '__main__':
    ...
    for sbj in Subject_ids:
        ...
        w_data_1 = w_data[:, :, 768:1280]

        subject_features0 = []
        u = 0
        for trial in w_data:
            subject_features0.append(mfcc_f(trial, sample_rate, winlen, winstep, nfilt, nfft, lowfreq, highfreq))
            u += 1
        w_data_f, w_labels = balanced_subsample(np.array(subject_features0), w_labels)  # downsampling to ensure equal number of targets.



        v_data = eeg_to_3d(v_data, epoch, int(v_data.shape[1] / epoch), n_chan)
        v_data_1 = v_data[:, :, 512:1024]
        subject_features0 = []
        u1 = 0
        for trial in v_data_1:
            subject_features0.append(mfcc_f(trial, sample_rate, winlen, winstep, nfilt, nfft, lowfreq, highfreq))
            u1 += 1
        v_data_f, v_labels_f = balanced_subsample(np.array(subject_features0), v_labels)
...
#####Compute indices for each word and vowel in the dataset#####
        ...
        for i, p in enumerate(word_id):
            ...
            writer = f"C:/Users/SB00763936/Desktop/EEG/imagined_speech_cnn/results/S{subject_id}/{p}.xlsx"
            ...
            for j in range(len(vowel_indices)):
                ...
                run_model = network_model(X_Train, X_Test, y_train, y_test, n_chans, input_time_length, cuda, args)
