...
def network_model(X_Train, X_Test, y_train, y_test, n_chans, input_time_length, cuda, args, p, j):
    max_epochs = 30
    ...
    # model_test.run()
    model_test = test(model=eval_model, data=(x_test, y_test), args=args, p=p, j=j)
    return model_test
    ...
    for sbj in Subject_ids:
        ...
        for i, p in enumerate(word_id):
            ...
            for j in range(len(vowel_indices)):
                ...
                run_model = network_model(X_Train, X_Test, y_train, y_test, n_chans, input_time_length, cuda, args, p,j)
