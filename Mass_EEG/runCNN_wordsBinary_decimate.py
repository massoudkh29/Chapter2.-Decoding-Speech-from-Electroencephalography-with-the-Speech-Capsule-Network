...
import pandas as pd
import numpy as np
from Mass_EEG.utils import load_subject_eeg, eeg_to_3d, return_indices, Ciaran, Experiment,balanced_subsample
...
if __name__ == '__main__':
    ...
    for sbj in Subject_ids:
        ...
        # w_data, _, w_labels, _ = load_subject_eeg(subject_id, vowels=False)
        ...
        v_data, v_labels = balanced_subsample(v_data, v_labels)
        #-------------------downsample?????????????
        w_data = w_data[:, :, 768:1280]
        # v_data = v_data[:, :, 512:1024]
        # w_data = decimate(w_data, 4, n=None, ftype='iir', axis=- 1, zero_phase=True)
        #------------------------------------------
