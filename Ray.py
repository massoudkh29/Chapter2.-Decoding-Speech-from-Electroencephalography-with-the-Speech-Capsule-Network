...
def train_mnist(config):
    ...
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Dense(num_classes, activation=config["activation"])
    ])
...
def tune_mnist(num_training_iterations):
    ...
    analysis = tune.run(train_mnist,
                        name="exp", scheduler=sched, metric="mean_accuracy", mode="max",
                        stop={"mean_accuracy": 0.99, "training_iteration": num_training_iterations},
                        num_samples=10, resources_per_trial={"cpu": 1, "gpu": 1},
                        config={
                            "activation": tune.choice(["relu", "tanh"]),
                            # "threads": 2,
                            "lr": tune.uniform(0.001, 0.1),
                            "momentum": tune.uniform(0.1, 0.9),
                            "hidden": tune.randint(32, 600),
                        })
...
if __name__ == "__main__":
    ...
    parser.add_argument("--server-address",type=str,default=None,required=False,
